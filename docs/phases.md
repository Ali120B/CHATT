...
| 7 | Complete | Invite codes, request/accept/reject/block/remove, friend-gated messaging. Asks/answers retry in background (survive restarts + offline peers); asks from unknown devices stash undecrypted and surface on invite import, so codes work in either order. |
